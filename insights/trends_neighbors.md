...
6. **对新入场者的建议**：不要做「GFlowNet vs X 谁更强」的对比论文（结论已明确且依场景而定）。要做的是把 GFN 的鲁棒性优势落到具体流水线（药物发现、LLM 后训练的覆盖率问题），做 GFN×OT 这类结构性连接（O08、N041 是抓手），或者在 FlowPlace 让出的问题上正面回答「为什么不用确定性流轨迹」。
