...
\subtitle{从流匹配到可认证训练：206 篇文献的结构化盘点}
...
\begin{frame}{本次调研的交付物}
  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{block}{四类产物}
        \begin{itemize}
          \item \textbf{206} 篇论文分类目录（TSV + markdown）
          \item \textbf{35} 篇核心论文中文深度解读
          \item \textbf{33} 篇保版式中文翻译 PDF（另 2 篇超长参考材料译核心章节）
          \item \textbf{3} 份趋势核实报告
        \end{itemize}
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{证据纪律}
...
\begin{frame}
  \begin{center}
    \Large 完整目录、逐篇解读与中英文 PDF 见仓库 \texttt{awesome\_Gflow}
  \end{center}
\end{frame}
